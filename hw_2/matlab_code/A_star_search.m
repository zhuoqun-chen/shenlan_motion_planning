...
%This part is about map/obstacle/and other settings
    ...
    CLOSED
    obs_map = obstable_map(MAP, MAX_X, MAX_Y)
    % obs_map
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while(1) %you have to dicide the Conditions for while loop exit 
        ...
        %--------------------------------
        exp_array=expand_array(xNode,yNode,path_cost,xTarget,yTarget,CLOSED,MAX_X,MAX_Y, obs_map)
        ...
        if(i_min ~= -1)
            ...
            if ( i_min_x==xTarget && i_min_y==yTarget )
                ...
                xNode = i_min_x
                yNode = i_min_y
                path_cost = OPEN(i_min, 7)%zqchen: need to update path_cost
                str = ['被弹出的节点的g(n) = ' num2str(path_cost)];disp(str);
                str = ['当前的CLOSED为: '];disp(str);
                disp(CLOSED);
                disp(['行数为 ' num2str(size(CLOSED, 1))]);
            end
        end
